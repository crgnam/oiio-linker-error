...
## Success Build on Windows with Visual Studio (works fine:

- Have vcpkg installed and integrated
- Allow VS to run CMake Configuration and Build All.  Completes sucessfully:
...
## Using apt instead of vcpkg:
If instead of using vcpkg, we install OpenImageIO using: `sudo apt install libopenimageio-dev`, and then change the `CMakeLists.txt` to use `find_library()` instead of `find_package()` everything works correctly on Ubuntu.  (See commented lines in `CMakeLists.txt`).

I feel as if this implies an issue with the vcpkg port.
